...
public abstract class MessageRouter {
	...
	/** Pass to MessageCCNStatsReport the information about messages transferred (interest and content)
	 * which needs the manipulation by protocol to inform if that was delivered to destination
	 *    
	 * */
	protected void ccnMessageTransferredListener(Message aMessage, DTNHost from, boolean isFirstDelivery) {
		for (MessageListener ml : this.mListeners) {
			ml.messageTransferred(aMessage, from, this.host,
				isFirstDelivery);
		}
	...
	protected Message removeFromMessages(String id) {
		Message m = null;
		while(this.hasMessage(id)){
			m = this.messages.remove(id);
		}
		return m;
	}
	...
	public RoutingInfo getRoutingInfo() {
		...
		RoutingInfo incoming = new RoutingInfo(this.incomingMessages.size() + 
				" incoming message(s)");
		RoutingInfo buffer = new RoutingInfo(this.messages.size() + 
				" carried message(s)");
